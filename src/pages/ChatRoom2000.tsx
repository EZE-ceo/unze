...
const ChatRoom2000 = () => {
  ...
  const roomYear = '2000';
  const roomDateText = '2000. 06. 22 오후 21:00, 서울';

  // ✅ 마지막 유저 퇴장 시 전체 메시지 삭제 함수
  const clearRoomMessagesIfLastOne = async (roomYear = '2000') => {
  ...
  useEffect(() => {
    ...
    // ✅ 무조건 입장 메시지 남기기 (중복 방지 없이, 들어올 때마다)
  ...
  const handleExit = async () => {
    ...
    // 퇴장 처리(위 useEffect 내부 exitRoom()에서도 처리)
    navigate('/MyPage');
  };

  // 구조(기존 스타일 className 맞춤)
  return (
    <div className="chatroom2000-wrapper">
      <div style={{ width: '100%', maxWidth: 420, height: '100%', maxHeight: 750, display: 'flex', flexDirection: 'column', background: '#BFBFBF', border: '1px solid #888', boxShadow: '0 0 6px rgba(0,0,0,0.3)' }}>
        ...
        <div className="header-bar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span className="header-text">2000. 06. 22 오후 21:00, 서울</span>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <button
              onClick={() => navigate('/report/2000')}
              style={{
                background: 'none',
                border: 'none',
                fontSize: '16px',
