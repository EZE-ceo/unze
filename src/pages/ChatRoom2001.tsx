...
const ChatRoom = () => {
  ...
  const clearRoomMessagesIfLastOne = async (targetRoomYear = roomYear) => {
    ...
    console.log(`[${targetRoomYear}] onlineUserCount:`, onlineUserCount);
  
    if (onlineUserCount === 0) {
      try {
        const messagesRef = collection(db, `chatrooms/${targetRoomYear}/messages`);
        const msgsSnapshot = await getDocs(messagesRef);
        console.log(`[${targetRoomYear}] 삭제 시도, messages 수:`, msgsSnapshot.size);
        ...
        console.error(`[${targetRoomYear}] 메시지 삭제 중 오류:`, err);
      }
    } else {
      console.log(`[${targetRoomYear}] onlineUserCount=0이 아님, 삭제 안함`);
    }
  };  
