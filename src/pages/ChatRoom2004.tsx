...
import {
  ...
  onSnapshot, orderBy, writeBatch, getDoc // ✅ 추가
...
const ChatRoom = () => {
  ...
  // [2] 입장/퇴장 중복방지 + 마지막 유저 체크
  const hasEnteredRef = useRef(false);

  useEffect(() => {
    if (hasEnteredRef.current || !nickname) return;
    ...
    const enterRoom = async () => {
      ...
      // ✅ 중복 입장 방지
      if (current?.isOnline && current?.year === roomYear) return;
    ...
    const exitRoom = async () => {
      ...
      // ✅ Firestore 반영 대기 (안정성 보장 위해 2.5초 후 실행)
      setTimeout(async () => {
        // ⛔ 내가 퇴장 처리된 이후의 online 유저 수를 확인
        const usersSnapshot = await getDocs(query(
          collection(db, "users"),
          where("year", "==", roomYear),
          where("isOnline", "==", true)
        ...
        if (onlineUserCount === 0) {
          ...
          console.log(`👥 ${onlineUserCount}명 남아있음. 메시지 유지됨.`);
        }
      }, 2500); // Firestore 반영 대기 시간
    };
